...
//        }
        try {
            listsURL = new ResourceReference(defURL);
            ...
            if (avoidOverlapingAnnotations) {
                ...
                if (!wholeWordsOnly) {
                    if (prevDistance <= distanceThreshold) {
                        int startOffset = 0;
                        if (prevDistance < distanceThreshold)
                            startOffset -= (int) ((distanceThreshold - prevDistance) / insertCost);
...
//                            } 
    ...
    public void setDefURL(URL defURL) {
        this.defURL = defURL;
    }
} // class BWPGazetteer
